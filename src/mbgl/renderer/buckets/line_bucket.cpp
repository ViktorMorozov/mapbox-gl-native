    ...
    for (std::size_t i = first; i < len; ++i) {
        ...
        if (isSharpCorner && i > first) {
            ...
            if (prevSegmentLength > 2.0 * sharpCornerOffset) {
                ...
                addCurrentVertex(newPrevVertex, distance, *prevNormal, 0, 0, false, startVertex, triangleStore, totalDistance, startDistance);
        ...
        if (middleVertex && currentJoin == LineJoinType::Miter) {
            ...
            if (currentJoin == LineJoinType::FakeRound) {
                ...
                for (int m = 0; m < n; m++) {
                    ...
                    addPieSliceVertex(*currentCoordinate, distance, approxFractionalJoinNormal, lineTurnsLeft, startVertex, triangleStore, totalDistance, startDistance);
                }

                addPieSliceVertex(*currentCoordinate, distance, joinNormal, lineTurnsLeft, startVertex, triangleStore, totalDistance, startDistance);
        ...
        if (isSharpCorner && i < len - 1) {
            ...
            if (nextSegmentLength > 2 * sharpCornerOffset) {
                ...
                addCurrentVertex(newCurrentVertex, distance, *nextNormal, 0, 0, false, startVertex, triangleStore, totalDistance, startDistance);
...
void LineBucket::addCurrentVertex(const GeometryCoordinate& currentCoordinate,
    ...
    if (distance > MAX_LINE_DISTANCE / 2.0f && !totalDistance && !startDistance) {
        distance = 0;
        addCurrentVertex(currentCoordinate, distance, normal, endLeft, endRight, round, startVertex, triangleStore, totalDistance, startDistance);
    }
}
